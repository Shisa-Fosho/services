...
## Convention File Mirror

`CLAUDE.md` and `AGENTS.md` are mirrors — they must hold identical content. Likewise, every file in `.claude/rules/` has an identical counterpart in `.codex/rules/`. **If you edit any of these files, you MUST apply the same edit to its counterpart in the same commit.** The point is convention consistency across tools: future sessions get the same rules whether the operator is using Claude Code or Codex.

This mirror requirement applies ONLY to project-memory and convention files. Tool-specific harness files — `.claude/{settings,hooks,skills,agents,worktrees}/`, Codex equivalents — are not mirrored because they configure tool-specific behavior with no cross-tool equivalent.
