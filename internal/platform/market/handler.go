...
func (handler *Handler) RegisterAdminRoutes(mux *http.ServeMux, adminMiddleware func(http.Handler) http.Handler) {
	...
	mux.Handle("POST /admin/events/{id}/binary/markets", adminMiddleware(http.HandlerFunc(handler.addBinaryMarket)))
	mux.Handle("POST /admin/events/{id}/neg-risk/markets", adminMiddleware(http.HandlerFunc(handler.addNegRiskMarket)))
